...
class Article < Content
  ...
  has_many :comments,   :dependent => :destroy, :order => "created_at ASC" do
    #def self.merge(merged)
      #Article.new(:description => self.body + Article.find_by_id(merged).body, :postid =>self.id.to_s)
    #end
  ...
  def merge_arts(mid)
    merged1 = Article.find_by_id(mid)
    self.body += "\n" + merged1.body
    merged1.comments.each do |com|
      new = com.clone
      new.save
      self.comments << new
    end 
