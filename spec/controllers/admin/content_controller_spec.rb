...
describe Admin::ContentController do
  describe 'merging articles' do
    ...
    it 'should call the merge method with the correct artcile ID' do
      #@article1 = Factory.create(:article, :id => '1')
      Factory(:blog)
      @orig = Factory(:article)
      @merged = Factory(:article)
      @user = Factory(:user, :text_filter => Factory(:markdown), :profile => Factory(:profile_admin, :label => Profile::ADMIN))
      request.session = {:user => @user.id}
      #ApplicationController.any_instance.stub(:load_lang)
      #ApplicationController.any_instance.stub(:set_paths)
      Article.any_instance.should_receive(:find_by_id).with(@orig.id).and_return(@orig)
      @orig.should_receive(:merge_arts).once
      post :merge, {:oid => @orig.id, :merge_with => @merged.id}
    end 
    it 'should render the merged article show view if successful'
    it 'should display an error if incorrect article id(optional)'
  end
  render_views
